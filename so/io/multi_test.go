...
func TestMultiReaderAsWriterTo(t *testing.T) {
	...
	mr2 := NewMultiReader(&sr2, &sr3)
	mr := NewMultiReader(&sr1, &mr2)
	mrAsWriterTo := WriterTo(&mr)
...
func TestMultiWriter_WriteStringSingleAlloc(t *testing.T) {
	...
	mw := NewMultiWriter(simpleWriter{&sink1}, simpleWriter{&sink2})
...
func TestMultiReaderFlatten(t *testing.T) {
	...
	mr := NewMultiReader(readerFunc(func(p []byte) (int, error) {
		n := runtime.Callers(1, pc)
...
func TestMultiReaderSingleByteWithEOF(t *testing.T) {
	mr := NewMultiReader(byteAndEOFReader('a'), byteAndEOFReader('b'))
	lr := LimitReader(&mr, 10)
	got, err := ReadAll(nil, &lr)
	if err != nil {
		t.Fatal(err)
	}
	const want = "ab"
...
func TestMultiReaderFinalEOF(t *testing.T) {
	...
	r := NewMultiReader(&br, byteAndEOFReader('a'))
	buf := make([]byte, 2)
	n, err := r.Read(buf)
	if n != 1 || err != EOF {
...
func TestInterleavedMultiReader(t *testing.T) {
	...
	mr1 := NewMultiReader(&r1, &r2)
	mr2 := NewMultiReader(&mr1)
