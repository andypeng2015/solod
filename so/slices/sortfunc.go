...
func insertionSort_func(data Sorter, a, b int) {
	for i := a + 1; i < b; i++ {
...
func siftDown_func(data Sorter, lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func heapSort_func(data Sorter, a, b int) {
	first := a
	lo := 0
	hi := b - a
...
func pdqsort_func(data Sorter, a, b, limit int) {
...
func partition_func(data Sorter, a, b, pivot int) (int, bool) {
	data.Swap(a, pivot)
...
// Returns newpivot.
func partitionEqual_func(data Sorter, a, b, pivot int) int {
	data.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
func partialInsertionSort_func(data Sorter, a, b int) bool {
	const (
...
func breakPatterns_func(data Sorter, a, b int) {
	length := b - a
	if length >= 8 {
...
//
// Return (pivot, hint)
func choosePivot_func(data Sorter, a, b int) (int, sortedHint) {
	const (
		shortestNinther = 50
		maxSwaps        = 4 * 3
	)
...
func order2_func(data Sorter, a, b int, swaps *int) (int, int) {
	if data.Less(b, a) {
		*swaps = *swaps + 1
		return b, a
	}
...
func median_func(data Sorter, a, b, c int, swaps *int) int {
...
func medianAdjacent_func(data Sorter, a int, swaps *int) int {
	return median_func(data, a-1, a, a+1, swaps)
}

func reverseRange_func(data Sorter, a, b int) {
	i := a
	j := b - 1
	for i < j {
		data.Swap(i, j)
...
func stable_func(data Sorter, n int) {
...
func symMerge_func(data Sorter, a, m, b int) {
...
func rotate_func(data Sorter, a, m, b int) {
	i := m - a
	j := b - m
